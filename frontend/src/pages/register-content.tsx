            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  If empty, will use backend wallet address from environment
                </p>
                {isConnected && address && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Connected wallet: {address.slice(0, 6)}...
                    {address.slice(-4)} (not used for registration)
                  </p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full min-h-[100px] px-3 py-2 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-md text-foreground placeholder:text-muted-foreground resize-y"
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                />
                <p className="text-xs text-muted-foreground">
                  This description will be stored in IPFS metadata
                </p>
              </div>
              <GlowButton
